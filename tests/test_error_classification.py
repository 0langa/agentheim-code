...
def test_provider_http_status_uses_status_mapping() -> None:
    ...
    assert classify_error(ProviderError("bad credentials", http_status=401)) == ErrorCategory.CONFIGURATION
    assert classify_error(ProviderError("forbidden", http_status=403)) == ErrorCategory.PERMISSION
    assert classify_error(ProviderError("missing deployment", http_status=404)) == ErrorCategory.CONFIGURATION
    assert classify_error(ProviderError("unprocessable", http_status=422)) == ErrorCategory.CONFIGURATION
...
def test_provider_error_without_http_status_keeps_string_fallback() -> None:
    ...
    assert classify_error(ProviderError("permission denied by provider")) == ErrorCategory.PERMISSION
